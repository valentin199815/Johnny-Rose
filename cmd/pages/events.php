<div class="event">
    ...
    <?php
        if($_SERVER['REQUEST_METHOD'] == "POST"){
            ...
            $file = "./pages/evenimg/".$_FILES['myfile']['name'];
            $picture_tmp = $_FILES['myfile']['tmp_name'];

            $targetdir = './pages/evenimg/'.basename($_FILES['myfile']['name']);
            ...
            $result = $connectdb->query($selectdb);
                if($result->num_rows>0){
                    while($row = $result->fetch_assoc()){
                        ...
                        echo "<td><a type='button' href='?addr=updateevent&updateeventid=".$eventid."'>Update</a></td>";
                        echo "<td><a type='button' href='?addr=delete&deleteeventid=".$eventid."'>Delete</a></td>";
                    }
                }
            $connectdb->close();
        ?>
